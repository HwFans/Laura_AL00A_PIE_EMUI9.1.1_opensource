...
static int get_sub_file_name(hisee_img_file_type type, char *sub_file_name, size_t max_len)
{
	int ret;
	...
	case SLOADER_IMG_TYPE:
		ret = memcpy_s(sub_file_name, max_len,
					   HISEE_IMG_SLOADER_NAME, sizeof(HISEE_IMG_SLOADER_NAME));
		if (ret != EOK) {
	...
	case COS_IMG_TYPE:
		ret = memcpy_s(sub_file_name, max_len,
						HISEE_IMG_COS_NAME, sizeof(HISEE_IMG_COS_NAME));
		if (ret != EOK) {
			pr_err("%s(): memcpy_s err.\n", __func__);
			return HISEE_SECLIB_ERROR;
		}
	...
	case OTP1_IMG_TYPE:
		ret = memcpy_s(sub_file_name, max_len,
						HISEE_IMG_OTP0_NAME, sizeof(HISEE_IMG_OTP0_NAME));
		if (ret != EOK) {
			pr_err("%s(): memcpy_s err.\n", __func__);
			return HISEE_SECLIB_ERROR;
		}
	...
	case MISC4_IMG_TYPE:
		ret = memcpy_s(sub_file_name, max_len,
					HISEE_IMG_MISC_NAME, sizeof(HISEE_IMG_MISC_NAME));
		if (ret != EOK) {
			pr_err("%s(): memcpy_s err.\n", __func__);
			return HISEE_SECLIB_ERROR;
		}
...
int filesys_hisee_read_image(hisee_img_file_type type, char *buffer)
{
	...
	(void)memset_s((void *)&local_img_header, sizeof(hisee_img_header), 0, sizeof(hisee_img_header));
	...
	ret = memcpy_s((void *)&local_img_header, sizeof(hisee_img_header),
			(void *)&(g_hisee_data.hisee_img_head), sizeof(hisee_img_header));
	if (ret != EOK) {
		pr_err("%s(): memcpy_s err.\n", __func__);
		mutex_unlock(&g_hisee_data.hisee_mutex);
