...
struct hisp_mdc_device hisp_mdc_dev;
void mdc_addr_release(unsigned int size)
{
	struct hisp_mdc_device *dev = &hisp_mdc_dev;

	hisi_fstcma_free(dev->mdc_virt_addr,dev->mdc_dma_addr,MEM_MDC_SIZE);
	dev->isp_mdc_count = 0;
	dev->mdc_dma_addr  = 0;
	dev->share_fd      = 0;
	dev->devbuf        = NULL;
	dev->mdc_virt_addr = NULL;
}
...
static void *get_fstcma_mdc(unsigned int size)
{
	struct hisp_mdc_device *dev = &hisp_mdc_dev;
	void *mdc_virt_addr = NULL;
	...
	if ((mdc_virt_addr = hisi_fstcma_alloc(&dev->mdc_dma_addr, size, GFP_KERNEL)) == NULL) {
		pr_err("[%s] mdc_virt_addr.%pK\n", __func__, mdc_virt_addr);
		return NULL;
	}
	return mdc_virt_addr;
}
...
int mdc_addr_pa_init(void)
{
	...
	dev->mdc_virt_addr = get_fstcma_mdc(MEM_MDC_SIZE);
	if(dev->mdc_virt_addr ==NULL)
	{
		pr_err("[%s] mdc_virt_addr.%pK\n", __func__, dev->mdc_virt_addr);
		return -ENOMEM;
	}
	return 0;
}
