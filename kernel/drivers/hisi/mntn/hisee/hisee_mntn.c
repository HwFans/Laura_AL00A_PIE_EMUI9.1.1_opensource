...
void rdr_hisee_save_print_log(void)
{
	...
	ret = snprintf_s(path, HISEE_MNTN_PATH_MAXLEN, (size_t)HISEE_MNTN_PATH_MAXLEN, "%s", PATH_ROOT);
	...
	ret = snprintf_s(path, HISEE_MNTN_PATH_MAXLEN, (size_t)HISEE_MNTN_PATH_MAXLEN,
	...
	pfn_cb_dump_done pfn_cb)
{
	...
	ret = snprintf_s(path, HISEE_MNTN_PATH_MAXLEN,
				(unsigned long)HISEE_MNTN_PATH_MAXLEN, "%s/%s", pathname, HISEE_LOG_FLIENAME);
	if (ret == HISEE_SECLIB_ERROR) {
	...
	ret = snprintf_s(debug_cont, HISEE_MNTN_PATH_MAXLEN,
				(unsigned long)HISEE_MNTN_PATH_MAXLEN,
