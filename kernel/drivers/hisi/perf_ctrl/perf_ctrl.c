...
err:
	...
	case PERF_CTRL_SET_FRAME_RATE:
		...
		ret = sched_set_group_window_size(DEFAULT_RT_FRAME_ID, frame_rate); //lint !e732
		break;
	...
	case PERF_CTRL_SET_RTG_LOAD_MODE:
		if (copy_from_user(&load_mode, uarg, sizeof(struct rtg_load_mode))) {
			pr_err("set_rtg_load_mode copy_from_user fail.\n");
			return -EFAULT;
		}

		ret = sched_set_group_load_mode(load_mode.grp_id,
				load_mode.freq_enabled,
				load_mode.util_enabled);
		break;
	default:
