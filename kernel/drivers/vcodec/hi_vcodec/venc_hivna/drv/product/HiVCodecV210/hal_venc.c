...
HI_S32 VENC_HAL_ResetReg()
{
    ...
    if (Venc_Regulator_Disable() != HI_SUCCESS)
        s32Ret = HI_FAILURE;

    if (Venc_Regulator_Enable() != HI_SUCCESS)
        s32Ret = HI_FAILURE;
...
{
	VeduEfl_DTS_CONFIG_S dts_cfg;
	S_HEVC_AVC_REGS_TYPE *pAllReg = NULL;
	HI_U64 phy_pgd_base;
	pAllReg = (S_HEVC_AVC_REGS_TYPE*)VeduIpCtx.pRegBase;

	venc_get_dts_info(&dts_cfg);
	phy_pgd_base = dts_cfg.SmmuPageBaseAddr;
